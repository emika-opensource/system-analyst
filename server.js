...
app.get('/api/specs/:id/comments', (req, res) => {
  const specs = loadJSON('specs.json', []);
  const spec = specs.find(s => s.id === req.params.id);
  if (!spec) return res.status(404).json({ error: 'Not found' });
...
app.post('/api/specs/:id/comments', (req, res) => {
  const specs = loadJSON('specs.json', []);
  const spec = specs.find(s => s.id === req.params.id);
  if (!spec) return res.status(404).json({ error: 'Not found' });
...
app.put('/api/specs/:id/comments/:commentId', (req, res) => {
  const specs = loadJSON('specs.json', []);
  const spec = specs.find(s => s.id === req.params.id);
  if (!spec) return res.status(404).json({ error: 'Spec not found' });
  const comment = (spec.comments || []).find(c => c.id === req.params.commentId);
...
app.delete('/api/specs/:id/comments/:commentId', (req, res) => {
  const specs = loadJSON('specs.json', []);
  const spec = specs.find(s => s.id === req.params.id);
  if (!spec) return res.status(404).json({ error: 'Spec not found' });
  spec.comments = (spec.comments || []).filter(c => c.id !== req.params.commentId);
  spec.updatedAt = new Date().toISOString();
  saveJSON('specs.json', specs);
  res.json({ success: true });
});
...
app.get('/view/:id', (req, res) => {
  ...
  const unresolvedComments = (spec.comments || []).filter(c => !c.resolved);
  const commentsHtml = unresolvedComments.length > 0 ? `
    <h2>Comments</h2>
    ${unresolvedComments.map(c => `<div style="margin-bottom:12px;padding:12px;background:#0f0f14;border-radius:8px;border-left:3px solid #6366f1">
      <div style="display:flex;justify-content:space-between;margin-bottom:4px">
        <span style="font-weight:600;font-size:0.85rem">${c.author || 'User'}</span>
        <span style="color:#888898;font-size:0.75rem">${new Date(c.createdAt).toLocaleDateString()}</span>
      </div>
      <div style="color:#a0a0b0;font-size:0.9rem">${c.text}</div>
    </div>`).join('')}
  ` : '';
